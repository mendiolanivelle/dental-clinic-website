...
export default function AdminSocialPage() {
  ...
  return <>
    ...
    <section className="mb-6 rounded-3xl bg-white p-5 soft-shadow sm:p-6">
      ...
      {!settings.aiConfigured && <p className="mt-4 rounded-2xl bg-[#fff0e7] p-4 text-xs font-bold text-[#914b22]">Set OPENAI_API_KEY in Coolify before enabling automatic publishing.</p>}
    ...
    <form className="rounded-3xl bg-white p-5 soft-shadow sm:p-6" onSubmit={save}>
      ...
      <div className="mt-6 grid gap-5 md:grid-cols-2">
        ...
        <label className="text-xs font-extrabold">Publishing starts (Manila hour)<input className="input-field mt-2" max="23" min="0" required type="number" value={form.postingStartHour} onChange={(event) => setForm({ ...form, postingStartHour: event.target.value })} /></label>
        <label className="text-xs font-extrabold">Publishing ends (Manila hour)<input className="input-field mt-2" max="23" min="0" required type="number" value={form.postingEndHour} onChange={(event) => setForm({ ...form, postingEndHour: event.target.value })} /></label>
      </div>
